...
class FaultInjectionAutoConfigurationTest {
    ...
    void backsOffWhenUserProvidesStrategyBean() {
        contextRunner.withUserConfiguration(UserStrategyConfig.class).run(ctx -> {
            assertThat(ctx).hasSingleBean(FaultDecisionStrategy.class);
            assertThat(ctx.getBean(FaultDecisionStrategy.class)).isSameAs(UserStrategyConfig.USER_INSTANCE);
        });
    }
