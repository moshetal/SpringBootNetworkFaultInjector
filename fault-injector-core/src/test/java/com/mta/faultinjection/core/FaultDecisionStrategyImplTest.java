...
class FaultDecisionStrategyImplTest {
    ...
    void globallyDisabledAlwaysPasses() {
        ...
        assertThat(strategy.decide(HttpMethod.GET, ANY_URI).instruction()).isEqualTo(FaultDecision.Instruction.PASS);
    }
    ...
    void probabilityUsesInjectedRandom() {
        ...
        assertThat(hits.decide(HttpMethod.GET, ANY_URI).instruction())
                .isEqualTo(FaultDecision.Instruction.INJECT_DELAY);
        assertThat(misses.decide(HttpMethod.GET, ANY_URI).instruction()).isEqualTo(FaultDecision.Instruction.PASS);
    }
    ...
    void telemetryHooksFireForMatchAndTrigger() {
        ...
        assertThat(events)
                .extracting(FaultInjectionEvent::outcome)
    ...
    @Test
    void outboundManagementUrlsNeverFaultEvenWhenRuleMatchesAll() {
        FaultInjectionProperties props = enabled();
        Rule r = delayRule("catch-all", null, 999);
        r.setUrlPattern(".*");
        props.getRules().add(r);
        FaultDecisionStrategyImpl strategy = alwaysTrigger(props);
        assertThat(strategy.decide(HttpMethod.GET, URI.create("http://localhost:8080/actuator/faultinjector"))
                        .instruction())
                .isEqualTo(FaultDecision.Instruction.PASS);
        assertThat(strategy.decide(HttpMethod.GET, URI.create("http://localhost:8080/fault-injector/api/config"))
                        .instruction())
                .isEqualTo(FaultDecision.Instruction.PASS);
    ...
    @Test
    void outboundExcludeCanBeDisabled() {
        FaultInjectionProperties props = enabled();
        props.setOutboundExcludeEnabled(false);
        Rule r = delayRule("catch-all", null, 999);
        r.setUrlPattern(".*");
        props.getRules().add(r);
        FaultDecisionStrategyImpl strategy = alwaysTrigger(props);
