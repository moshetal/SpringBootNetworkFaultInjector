...
import com.mta.faultinjection.exclusion.OutboundFaultUriExclusion;
...
public class FaultDecisionStrategyImpl implements FaultDecisionStrategy {
    ...
    private final OutboundFaultUriExclusion outboundExclusion;
     ...
     */
    public FaultDecisionStrategyImpl(
            FaultInjectionProperties properties, DoubleSupplier randomSupplier, FaultInjectionTelemetry telemetry) {
        this.properties = Objects.requireNonNull(properties, "properties");
        this.randomSupplier = Objects.requireNonNull(randomSupplier, "randomSupplier");
        this.telemetry = telemetry;
        this.outboundExclusion = new OutboundFaultUriExclusion(properties);
    }
    ...
    public FaultDecision decide(HttpMethod method, URI uri) {
        ...
        if (outboundExclusion.shouldExclude(uri)) {
            return FaultDecision.pass();
        }
    ...
    private boolean matches(Rule rule, HttpMethod method, URI uri) {
        if (!rule.safeMethods().isEmpty()
                && method != null
                && !rule.safeMethods().contains(method)) {
            return false;
        }
    ...
    private boolean shouldFire(Rule rule, RuleMetrics m) {
        TriggerMode mode = rule.getMode() != null
                ? rule.getMode()
                : properties.getDefaults().getMode();
        ...
        // PROBABILITY
        double p = rule.getProbability() != null
                ? rule.getProbability()
                : properties.getDefaults().getProbability();
        if (p <= 0.0d) {
            return false;
        }
        if (p >= 1.0d) {
    ...
    public static final class RuleMetrics {
        ...
        public Map<String, Long> asMap() {
            ...
            m.put(FaultInjectorViewJsonKeys.MATCH_COUNT, matchCount.get());
            m.put(FaultInjectorViewJsonKeys.TRIGGER_COUNT, triggerCount.get());
            return m;
        }
    }
}
