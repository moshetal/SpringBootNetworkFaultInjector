/*
 * Cluster console UI for the fault injector control server.
 */
...
function buildUrl(path) {
    if (path === '/overview') return `${API_BASE}/overview`;
    if (path === '/services') return `${API_BASE}/services`;
    if (!state.serviceName) throw new Error('No service selected');
    const sep = path.includes('?') ? '&' : '?';
    return `${API_BASE}/services/${encodeURIComponent(state.serviceName)}${path}${sep}scope=${encodeURIComponent(state.scope)}`;
}
...
async function loadServices() {
    const names = await api('GET', '/services');
    const sel = $('#service-select');
    sel.innerHTML = '';
    for (const n of names) {
        sel.appendChild(el('option', { value: n }, n));
    }
    if (names.length && !state.serviceName) {
        state.serviceName = names[0];
    }
    sel.value = state.serviceName || '';
    await loadInstances();
}

async function loadInstances() {
    if (!state.serviceName) return;
    const res = await fetch(`${API_BASE}/services/${encodeURIComponent(state.serviceName)}/instances`);
    if (!res.ok) {
        state.instances = [];
    } else {
...
function renderScopeRadios() {
    const host = $('#scope-radios');
    host.innerHTML = '';
    const allId = 'scope-all';
    host.appendChild(el('label', { className: 'flex items-center gap-1' }, [
        el('input', { type: 'radio', name: 'scope', id: allId, checked: state.scope === 'all', onChange: () => { state.scope = 'all'; refreshAll(); } }),
        'All instances',
    ]));
    for (const inst of state.instances) {
        host.appendChild(el('label', { className: 'flex items-center gap-1 font-mono text-xs' }, [
            el('input', {
                type: 'radio', name: 'scope', checked: state.scope === inst.instanceId,
                onChange: () => { state.scope = inst.instanceId; refreshAll(); },
            }),
            inst.instanceId,
        ]));
    }
...
async function refreshOverview() {
    try {
        const rows = await api('GET', '/overview');
        state.overview = rows;
        renderOverview(rows);
    } catch (err) {
        console.warn('[console] overview failed:', err);
    }
}
...
function renderOverview(rows) {
    ...
    tbody.innerHTML = '';
    if (!rows.length) {
        tbody.appendChild(el('tr', {}, [el('td', { colspan: '5', className: 'text-center text-slate-500 py-4' }, 'No services registered yet.')]));
        return;
    }
    for (const r of rows) {
        tbody.appendChild(el('tr', {}, [
            ...
            el('td', { className: 'text-right tabular-nums' }, fmtNumber(r.resilienceSignals)),
        ]));
    }
}
...
function renderConfig(cfg) {
    state.config = cfg;
    if (cfg.consistent === false) {
        $('#kpi-rules-total').textContent = '—';
        $('#kpi-rules-active').textContent = '—';
        $('#rules-list').innerHTML = '<div class="p-6 text-center text-sm text-amber-600">Configs differ across instances. Select a single pod to edit.</div>';
        return;
    }
    const rules = cfg.rules || [];
    $('#kpi-rules-total').textContent = String(rules.length);
    $('#kpi-rules-active').textContent = String(rules.filter(r => r.enabled).length);
    $('#kpi-mount-path').textContent = state.serviceName || '—';
...
async function refreshAll() {
    ...
    try {
        ...
        console.warn('[console] refresh failed:', err);
    }
}
...
function wireUp() {
    ...
    $('#export-csv-btn').addEventListener('click', () => { window.location.href = buildUrl('/export?format=csv'); });
    $('#export-json-btn').addEventListener('click', () => { window.location.href = buildUrl('/export?format=json'); });

    $('#service-select').addEventListener('change', async (ev) => {
